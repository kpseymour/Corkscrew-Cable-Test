...
void loop() {
  ...
  if (state == 0){
    if (i==0 && upperLimitSwitch == HIGH){
      ...
      elevatorDestination = elevatorDestination + (6*ticksPerInch);
      if (elevatorDestination > 6*ticksPerInch){
        elevatorDestination = 0;
      }
      Serial.println("upperLimitSwitch");
      Serial.println(elevatorDestination);
      delay(50);
    }
  ...
  if (state == 1){
    if (j==0 && lowerLimitSwitch == HIGH){
      ...
      twisterDestination = twisterDestination - ticksPerTwist;
      if (twisterDestination < -ticksPerTwist){
        twisterDestination = 0;
      }
      Serial.println("lowerLimitSwitch");
      Serial.println(twisterDestination);
      delay(50);
    }
  ...
  if (state == 2){
    if(twisterTriggerGo){
      if(goToTwisterSetpoint(twisterDestination)){
  ...
  if (state == 3){
    if(elevatorTriggerGo){
      if(goToElevatorSetpoint(elevatorDestination)){
